...
/** Zweisprachiges Feld: { de, en } */
export type Localized<T = string> = { de: T; en: T };

/** Feld, das entweder einsprachig (string) oder zweisprachig sein kann */
export type LocField<T = string> = T | Localized<T>;

export type Category = "filter" | "espresso" | "sets" | "zubehoer";
...
export interface Product {
  ...
  desc: Localized;
  facts: Localized<Record<string, string>>;
  roastLevel: Localized | null;
}
...
/** Lokalisiertes Feld auflösen (string bleibt string, {de,en} → Sprache) */
export function loc<T>(field: T | Localized<T> | null | undefined, lang: Lang): T {
  if (field == null) return "" as T;
  if (typeof field === "string") return field as T;
  const l = field as Localized<T>;
  return l[lang] != null ? l[lang] : l.de;
}
...
/** Artikel absteigend nach Datum */
export function sortedArticles(): Article[] {
  return [...articles].sort((a, b) => b.date.localeCompare(a.date));
}

export const SITE_URL = "https://www.marlow-roasters.de";
